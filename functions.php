<?php
/**
 * Digitalno Legalno — funkcije teme.
 *
...
function theme_setup() {
	load_theme_textdomain( 'digitalno-legalno', get_template_directory() . '/languages' );
	...
	register_nav_menus(
		array(
			'main_menu'       => esc_html__( 'MAIN', 'digitalno-legalno' ),
			'meta_menu'       => esc_html__( 'META', 'digitalno-legalno' ),
			'responsive_menu' => esc_html__( 'MOBILE', 'digitalno-legalno' ),
		)
	);
}
 ...
 * @return string
 */
function digitalno_legalno_blog_category_slug() {
	return apply_filters( 'digitalno_legalno_blog_category_slug', 'compliance-resursi' );
}
 ...
 */
function digitalno_legalno_category_hero_acf_context( $term ) {
 ...
 * @return WP_Term|null
 */
function digitalno_legalno_post_display_category() {
 ...
 * @return void
 */
function digitalno_legalno_blog_category_query_include_children( $query ) {
	if ( is_admin() || ! $query->is_main_query() || ! $query->is_category() ) {
		return;
	}

	$blog = get_term_by( 'slug', digitalno_legalno_blog_category_slug(), 'category' );
...
add_action( 'pre_get_posts', 'digitalno_legalno_blog_category_query_include_children' );
 ...
 * @return string
 */
function digitalno_legalno_posts_pagination_no_sr_heading( $html ) {
	if ( '' === $html ) {
		return $html;
	}
...
add_filter( 'get_the_posts_pagination', 'digitalno_legalno_posts_pagination_no_sr_heading' );

/**
 * Učitavanje WP Bootstrap Navwalker klase.
 *
 * @return void
 */
function digitalno_legalno_register_navwalker() {
	require_once get_template_directory() . '/class-wp-bootstrap-navwalker.php';
}
add_action( 'after_setup_theme', 'digitalno_legalno_register_navwalker' );

/**
 * BS5: data-bs-toggle umesto data-toggle na dropdown stavkama.
 *
 ...
 * @return array
 */
function digitalno_legalno_bs5_dropdown_data_attribute( $atts, $item, $args ) {
 ...
 * @return string
 */
function digitalno_legalno_wpcf7_ajax_loader() {
	return trailingslashit( get_stylesheet_directory_uri() ) . 'images/ajax-loader.gif';
}
add_filter( 'wpcf7_ajax_loader', 'digitalno_legalno_wpcf7_ajax_loader' );
...
if ( function_exists( 'acf_add_options_page' ) ) {
	...
	acf_add_options_page(
		array(
			'page_title' => 'Zakaži razgovor',
			'menu_title' => 'Zakaži razgovor',
			'menu_slug'  => 'book-call',
			'position'   => '9',
		)
	);
	...
	acf_add_options_page(
		array(
			'page_title' => 'Paketi usluga',
			'menu_title' => 'Paketi usluga',
			'menu_slug'  => 'packages',
			'position'   => '9',
		)
	);
 ...
 * @return void
 */
function digitalno_legalno_login_logo() {
...
add_action( 'login_enqueue_scripts', 'digitalno_legalno_login_logo' );
 ...
 * @return void
 */
function digitalno_legalno_hide_admin_menus() {
...
add_action( 'admin_menu', 'digitalno_legalno_hide_admin_menus', 120 );
 ...
 * @return void
 */
function digitalno_legalno_security_headers() {
	if ( headers_sent() ) {
		return;
	}
...
add_action( 'send_headers', 'digitalno_legalno_security_headers', 1 );
 ...
 * @return void
 */
function digitalno_legalno_remove_media_buttons() {
	remove_action( 'media_buttons', 'media_buttons' );
}
add_action( 'admin_head', 'digitalno_legalno_remove_media_buttons' );