# Digitalno Legalno — WordPress Theme

Custom WordPress theme for **[Digitalno Legalno](https://digitalno-legalno.rs/)** — a marketing and content site built with **Bootstrap 5**, **Sass**, and **Advanced Custom Fields (ACF)**. The front end is component-driven (`global-templates/`, `loop-templates/`) with a Serbian (`sr_RS`) translation domain.

**Theme identity**

| | |
|---|---|
| **Text domain** | `digitalno-legalno` |
| **Package (PHPDoc)** | `digitalno-legalno` |
...
- **ACF-driven content** — options pages for packages, services, FAQ, booking CTA, logo, and page-level heroes and sections
- **Custom page templates** — Front Page, Contact, Services, Packages, About, plus default `page.php` for editor-driven legal pages
...
| **Contact Form 7** | Kontakt template uses a shortcode-based form; replace the form ID in `loop-templates/content-contact.php` for your environment |
...
1. Copy the theme folder into your WordPress installation:
   ...
   `wp-content/themes/digitalno-legalno/`

2. In **Appearance → Themes**, activate **Digitalno Legalno**.

3. Install and activate **ACF** (and **Contact Form 7** if you use the contact page as shipped).

4. Recreate **ACF field groups** to match the theme (field names are documented in partials, e.g. `global-templates/services.php`, `packages.php`, `faq.php`, `facts.php`). Options are registered in `functions.php` under `acf_add_options_page` (slugs such as `general-settings`, `book-call`, `packages`, `services`, `faq`).
...
6. Set **Settings → Reading** so the desired page uses the **Front Page** template, and assign other pages to **Contact**, **Services**, **Packages**, or **About us** as needed.
...
```bash
cd wp-content/themes/digitalno-legalno
npm install
```
...
| `global-templates/` | Reusable partials (hero, packages, FAQ, bio, etc.) |
