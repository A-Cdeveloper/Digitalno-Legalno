...
$blog_url  = home_url( '/' );
$blog_term = get_term_by( 'slug', digitalno_legalno_blog_category_slug(), 'category' );
...
if ( $last_posts->have_posts() ) {
	while ( $last_posts->have_posts() ) {
		$last_posts->the_post();
		$display = function_exists( 'digitalno_legalno_post_display_category' )
			? digitalno_legalno_post_display_category()
			: null;
		$items[] = array(
			'tag'   => $display ? $display->name : __( 'Članak', 'digitalno-legalno' ),
			'title' => get_the_title(),
			'meta'  => get_the_date(),
			'url'   => get_permalink(),
		);
...
<section class="last-articles py-5 py-lg-6 py-xl-8 border-top reveal-on-scroll" aria-labelledby="last-articles-heading">
    <div class="container">
        ...
        <h2 id="last-articles-heading" class="section-heading">
            <?php esc_html_e( 'Poslednji članci', 'digitalno-legalno' ); ?>
        </h2>
        ...
        <p class="text-center mt-4 mt-lg-5 mb-0">
            <a class="btn btn-outline-primary" href="<?php echo esc_url( $blog_url ); ?>">
                <?php esc_html_e( 'Svi članci', 'digitalno-legalno' ); ?>
